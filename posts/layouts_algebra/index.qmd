...
* Tile-wise copy: each thread copies a tile of the source tensor to a tile of the destination tensor.
* Strided copy: each thread copies a single element of the source tensor to a single element of the destination tensor.
...
l = Layout(T(8, 8), T(1, 8))
p = LayoutPlotter(l, title="Source Tensor, tile copy")
...
Here different colors indicate the thread that is responsible for copying the data.To understand effect of different copy strategies, we can 
