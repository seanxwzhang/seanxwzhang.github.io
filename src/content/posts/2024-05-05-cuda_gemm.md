...
_In this post I introduce the CUDA programming ecosystem by writing a kernel achieves 95% of cuBLAS’s SGEMM performance. I hope this helps those are just getting started in CUDA programming._
...
2. Given a particular way to break down the problem, what’s the bottleneck of my current solution?
...
SM: Streaming Multiprocessor, the first-level “parallel component” in GPU, there’re 128 SM in a RTX4090
HBM: High Bandwidth Memory, the “global memory”/”device memory” of GPU, size is usually around 20GB~80GB. Communicating with this memory is considered extremely slow, on the order of 400-800 cycles per access.
L2 cache: The cache for HBM, read and writes to HBM will be cached here. Communication with this memory is much faster (but Nvidia doesn’t disclose it publicly), a ballpark number for  is 2TB/s.
SMSP: Streaming Multiprocessor Sub-Partition, introduced in the Ampere architecture, the second-level “parallel component” in GPU, there’re 4 SMSP in an SM.
...
INT32: The arithmetic “core” that performs 32-bit integer arithmetics. This core works on scalar values. 
FP32: The arithmetic “core” that performs 32-bit floating point arithmetics. This core works on scalar values.
Tensor Cores: The arithmetic “core” that can both perform floating point arithmetics and integer arithmetics. This core works on a bunch of values at a time (hence the name tensor)](/images/posts/cuda_gemm-23.png)
...
1. Both L1 cache and L2 cache are “invisible” to programmers in the sense that you can’t directly manipulate them (but you can [set the persistence size of L2 cache](https://docs.nvidia.com/cuda/cuda-c-programming-guide/index.html#device-memory-l2-access-management), or [give hints](https://docs.nvidia.com/cuda/parallel-thread-execution/index.html#cache-operators) to the machine w.r.t. the cache policy)
2. L1 cache and Shared Memory actually the same physical memory, it’s up to the programmer to configure the size of each.
...
![Warp is the smallest scheduling primitive, an SMSP can only execute instructions for one warp at a time. The “.add” here simply symbolizes an addition operation carried out by each warp.](/images/posts/cuda_gemm-24.png)
...
![A more realistic depiction of different warp carrying out memory loading before doing addition. ](/images/posts/cuda_gemm-25.png)
...
To get the numerator, we have to profile the kernel and obtained from measuring the warp scheduler’s issuance statistics. Usually this done via Nvidia’s [nsight compute](https://developer.nvidia.com/nsight-compute) (in the old days it’s nvprof, but that’s long gone).
...
![Naive matmul](/images/posts/cuda_gemm-26.png)
...
![As can be seen, the arithmetic intensity of this kernel is too low, resulting 6% of theoretical fp32 performance achieved](/images/posts/cuda_gemm-27.png)
...
![This naive kernel achieves full throughput in both memory and compute! but it’s very bad algorithm](/images/posts/cuda_gemm-7.png)
...
BK is the parameter determining how much data we load from global memory to shared memory every time. We have to have this BK parameter because shared memory is very limited, a whole BMxK block of data usually doesn’t fit](/images/posts/cuda_gemm-28.png)
...
![Thread tiling + Shared memory access](/images/posts/cuda_gemm-29.png)
...
![Data movement rate comparison between different size of element read per thread [6]](/images/posts/cuda_gemm-30.png)
...
Here the region marked by t1 and t2 are indicating what data each thread is responsible of loading from global memory and writing to shared memory. In practice, a thread can handle less than a row in the sub-block of global memory.](/images/posts/cuda_gemm-31.png)
...
![Occupancy is high](/images/posts/cuda_gemm-33.png)
...
![Diagram illustrating bank conflict, source](/images/posts/cuda_gemm-34.png)
...
![Diagram illustrating 4-way bank conflict](/images/posts/cuda_gemm-35.png)
...
![Diagram illustrating swizzle operation’s outcome](/images/posts/cuda_gemm-17.png)
...
![Share memory conflict significantly reduced, it didn’t reduce to 0 because of reason that will become clear in the subsequent section](/images/posts/cuda_gemm-18.png)


However, the performance is completely disastrous


![Performance of prefetching + swizzle goes back to round 1](/images/posts/cuda_gemm-37.png)
...
2. A strided memory access pattern (from a thread’s perspective) allows for coalesced memory access.
3. Most importantly, because we are doing more work per thread (9 thread-tiles per thread, in the example), we are again increasing the arithmetic intensity!
    1. From shared memory’s perspective, processing each $TM\times TN$ elements still requires $2K$ reads from shared memory
    2. But from device memory’s perspective, processing each $\#\text{subtiles}\times TM \times TN$ elements only requires $\text{sm}\times TM \times K + \text{sn}\times TN \times K$ reads from global memory, meaning the arithmetic intensity is now
...
![Diagram illustrating split-K](/images/posts/cuda_gemm-38.png)
...
2. I’m using the kernel launch as an inter-block synchronization primitive (basically, calling the reduction kernel after the computation kernel in the same stream), but ideally, reduction can also happen in a block-by-block fashion, where as long as a set of blocks that cover the entire K dimension has finished calculation, a reduction kernel can be kicked off along that dimension. I’m not sure how to achieve this though, because afaik CUDA doesn’t have any other inter-block synchronization primitive than kernel launching.
3. I’m not using fancy things like [stream-k](https://arxiv.org/abs/2301.03598).
4. I’m too exhausted to do a proper swizzle on this kernel.
...
[1] [CUDA Training Series – Oak Ridge Leadership Computing Facility (ornl.gov)](https://www.olcf.ornl.gov/cuda-training-series/)
